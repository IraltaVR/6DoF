...
public class BasicController : MonoBehaviour { 
	...
	private void Start() {
        ...
        PostProcessingBehaviour processingBehaviour = GetComponent<PostProcessingBehaviour>();
        // Profile from scratch
        //PostProcessingProfile profile = ScriptableObject.CreateInstance("PostProcessingProfile") as PostProcessingProfile;
        // Or modify current profile
         profile = processingBehaviour.profile;

        profile.vignette.enabled = hacerFade;
       g = profile.vignette.settings;
    }
    ...
    public void Fade()
    {
        if(transform.localPosition.x > distancia)
        {
            canvasGroup.alpha = transform.localPosition.x * sensibilidad;
        }
        else
        {
            if(transform.localPosition.x < -distancia)
            ...
            {
                if (transform.localPosition.z > distancia)
                {
                    canvasGroup.alpha = transform.localPosition.z * sensibilidad;
                }
                else
                {
                    if (transform.localPosition.z < -distancia)
    ...
    public void Vignette()
    {
        ...
        if (transform.localPosition.x > distancia)
        {
            g.opacity = transform.localPosition.x * sensibilidad;
        }
        else
        {
            if (transform.localPosition.x < -distancia)
            ...
            else
            {
                if (transform.localPosition.z > distancia)
                {
                    g.opacity = transform.localPosition.z * sensibilidad;
                }
                else
                {
                    if (transform.localPosition.z < -distancia)
